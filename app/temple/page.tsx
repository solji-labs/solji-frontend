...
import { getTempleLevel, getDonationLeaderboard } from "@/lib/api"
import type { TempleLevelResponse, DonationLeaderboardItem } from "@/lib/api/types"
...
export default function TempleHomePage() {
  ...
  const progress = useMemo(() => {
    ...
    const donationsPct = req && req.donations_sol > 0 ? Math.min(100, (donations / req.donations_sol) * 100) : 0
    const wishesPct = req && req.wishes > 0 ? Math.min(100, (wishes / req.wishes) * 100) : 0
    const fortunesPct = req && req.draw_fortune > 0 ? Math.min(100, (fortunes / req.draw_fortune) * 100) : 0
    const incensePct = req && req.incense_points > 0 ? Math.min(100, (incensePoints / req.incense_points) * 100) : 0
  ...
  function inscenceSafe(v: number) { return isFinite(v) ? v : 0 }
  function shortKey(k: string) { return k ? `${k.slice(0, 4)}...${k.slice(-4)}` : '' }

  const templeName = data?.level_name_en ? { cn: data.level_name, en: data.level_name_en } : { cn: '——', en: '' }
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold">
                  {templeName.cn} <span className="text-muted-foreground text-xl">Lv.{data?.current_level ?? '-'}</span>
                </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full lg:w-auto">
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{(data?.stats.total_draw_fortune ?? 0).toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Believers</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{(data?.stats.total_incense_points ?? 0).toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Incense</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{(data?.stats.total_fortune_nfts ?? 0).toLocaleString()}</div>
              <div className="text-xs text-muted-foreground">Fortunes</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-primary">{(data?.stats.total_donations_sol ?? 0)} SOL</div>
        ...
        <div className="mt-6 pt-6 border-t border-border">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold">
              Next Level: {nextLevel?.level_name ?? '—'} ({nextLevel?.level_name_en ?? ''})
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span>Incense</span>
                <span className="text-muted-foreground">{progress.incense}%</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span>Fortunes</span>
                <span className="text-muted-foreground">{progress.fortunes}%</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span>Wishes</span>
                <span className="text-muted-foreground">{progress.wishes}%</span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span>Donations</span>
                <span className="text-muted-foreground">{progress.donations}%</span>
              </div>
            ...
            {leaderboard.map((contributor) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium">{shortKey(contributor.user_pubkey)}</p>
                    <p className="text-xs text-muted-foreground">Donated</p>
                  </div>
                </div>
                <span className="text-sm font-semibold text-primary">{contributor.total_donated} SOL</span>
              </div>
            ))}
          </div>
        </Card>
